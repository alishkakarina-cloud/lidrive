...
export function BookingCta({ title, text, label = "Записаться" }: { title: string; text: string; label?: string }) {
  ...
  return (
    <div className="flex flex-col items-start justify-between gap-6 rounded-3xl border border-line bg-panel p-8 sm:flex-row sm:items-center sm:p-12">
      <div>
      ...
      <button
        ...
        className="group flex shrink-0 items-center gap-2 rounded-full bg-accent hover:bg-accent-hover px-7 py-3.5 text-sm font-semibold text-white transition hover:scale-[1.03] active:scale-95"
      >
        {label}
